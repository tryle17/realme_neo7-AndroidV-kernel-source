...
static atomic_t game_atomic_pid = ATOMIC_INIT(0);
...
static int reset_wts_stat(pid_t pid){
	atomic_set(&game_atomic_pid, pid);
	return 0;
}

static void flush_task(struct task_struct *p){
	pid_t tgid;
	tgid = atomic_read(&game_atomic_pid);
...
static void penalty_flush_task(void *unused, struct task_struct *p){
	flush_task(p);
}
...
static ssize_t get_target_pid(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	pid_t usr_val  = atomic_read(&game_atomic_pid);
...
#if 0
static ssize_t penalty_set_penalty_state(struct file *file,
			const char __user *buf, size_t count, loff_t *ppos)
{
	...
	ret = sscanf(page, "%d", &user_val);
	if (ret)
		return -EINVAL;

	atomic_set(&sysctl_sched_auto_penaty, user_val);

	return count;
}

static ssize_t penalty_get_penalty_state(struct file *file,
			char __user *buf, size_t count, loff_t *ppos)
{
	...
	user_val = atomic_read(&sysctl_sched_auto_penaty);
	len = sprintf(page, "%u", user_val);

	retrun simple_read_from_buffer(buf, count, ppos, page, len);
}

static const struct proc_ops penalty_state = {
	.proc_write		= penalty_set_penalty_state,
	.proc_read		= penalty_get_penalty_state,
	.proc_lseek		= default_llseek,
};
#endif

static ssize_t penalty_set_target_fps(struct file *file,
			const char __user *buf, size_t count, loff_t *ppos)
{
	int ret;
	long usr_val = 0;
...
static ssize_t penalty_get_target_fps(struct file *file,
			char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	long len, usr_val;
...
static const struct proc_ops penalty_target_fps_ops = {
	.proc_write		= penalty_set_target_fps,
	.proc_read		= penalty_get_target_fps,
	.proc_lseek		= default_llseek,
};

static ssize_t penalty_set_target_pid(struct file *file,
			const char __user *buf, size_t count, loff_t *ppos)
{
	long usr_val = 0;
	int ret;
...
static ssize_t penalty_get_target_pid(struct file *file,
			char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	long len;
...
static const struct proc_ops penalty_target_pid_ops = {
	.proc_write		= penalty_set_target_pid,
	.proc_read		= penalty_get_target_pid,
	.proc_lseek		= default_llseek,
};
...
static int init_module_params(void)
{
	...
	proc_create_data("penalty_target_fps", 0664, game_opt_dir,
			&penalty_target_fps_ops, NULL);

	proc_create_data("penalty_target_pid", 0664, game_opt_dir,
			&penalty_target_pid_ops, NULL);
#if 0
	proc_create_data("penalty_enable", 0664, game_opt_dir,
			&penalty_state, NULL);
#endif
